...
2. Run project tests/lint before pushing (follow repo conventions).
...
6. Write/update PR body:
   - If `.github/pull_request_template.md` exists, use it as the body
     structure: fill every section with concrete content, replace all
     placeholder comments (`<!-- ... -->`), keep bullets/checkboxes.
   - Otherwise, write a clear PR body with a summary of changes, rationale,
     and test plan.
...
# Run project validation (adapt to repo conventions)
# e.g.: make test, npm test, pytest, mix test, cargo test, etc.
...
# Write/edit PR body before pushing.
# If .github/pull_request_template.md exists, use it as the structure.
# Otherwise, write a clear body with summary, rationale, and test plan.
# Example workflow:
